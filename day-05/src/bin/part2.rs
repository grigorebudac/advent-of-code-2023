...
fn process(input: &str) -> u64 {
    ...
    let mut new_seeds_set = HashSet::new();

    for chunk in seeds.chunks(2) {
        if let [start, count] = chunk {
            new_seeds_set.extend(*start..=(*start + *count - 1));
        }
    }

    let new_seeds: Vec<u64> = new_seeds_set.into_iter().collect();

    let smallest_location = new_seeds
        .iter()
